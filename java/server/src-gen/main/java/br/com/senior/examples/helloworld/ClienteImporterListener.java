...
public class ClienteImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(ClienteImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  HelloWorldConstants.Events.IMPORT_CLIENTE_EVENT, new ImportClienteEventPayload(status));
		}
	}
