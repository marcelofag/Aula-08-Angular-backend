 ...
 * 
 */
public class UpdateMergeClienteImpl {
	...
	public UpdateMergeClienteImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Cliente input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				HelloWorldConstants.DOMAIN, //
				HelloWorldConstants.SERVICE, //
				HelloWorldConstants.Commands.UPDATE_MERGE_CLIENTE, //
	...
	*/
	public Cliente updateMergeCliente(Cliente input, long timeout) {
		...
		Cliente output = DtoJsonConverter.toDTO(resultMessage.getContent(), Cliente.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Cliente> updateMergeClienteRequest(Cliente input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Cliente.class);
	}
