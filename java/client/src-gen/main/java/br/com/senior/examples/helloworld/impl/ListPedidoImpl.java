...
public class ListPedidoImpl {
	...
	public ListPedidoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Pedido.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				HelloWorldConstants.DOMAIN, //
				HelloWorldConstants.SERVICE, //
				HelloWorldConstants.Commands.LIST_PEDIDO, //
	...
	*/
	public Pedido.PagedResults listPedido(Pedido.PageRequest input, long timeout) {
		...
		Pedido.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Pedido.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Pedido.PagedResults> listPedidoRequest(Pedido.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Pedido.PagedResults.class);
	}
