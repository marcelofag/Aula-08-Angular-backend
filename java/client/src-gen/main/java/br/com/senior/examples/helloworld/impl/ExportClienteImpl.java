 ...
 * 
 */
public class ExportClienteImpl {
	...
	public ExportClienteImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportClienteInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				HelloWorldConstants.DOMAIN, //
				HelloWorldConstants.SERVICE, //
				HelloWorldConstants.Commands.EXPORT_CLIENTE, //
	...
	*/
	public ExportClienteOutput exportCliente(ExportClienteInput input, long timeout) {
		...
		ExportClienteOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportClienteOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ExportClienteOutput> exportClienteRequest(ExportClienteInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportClienteOutput.class);
	}
