 ...
 * 
 */
public class ImportClienteImpl {
	...
	public ImportClienteImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportClienteInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				HelloWorldConstants.DOMAIN, //
				HelloWorldConstants.SERVICE, //
				HelloWorldConstants.Commands.IMPORT_CLIENTE, //
	...
	*/
	public ImportClienteOutput importCliente(ImportClienteInput input, long timeout) {
		...
		ImportClienteOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportClienteOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportClienteOutput> importClienteRequest(ImportClienteInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportClienteOutput.class);
	}
